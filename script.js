...
shareButton.addEventListener('click', (event) => {
    ...
    const encodedRepo = encodeURIComponent(displayRepo); // Encode for URL
    const encodedBranch = encodeURIComponent(displayBranch);
    const displayPath = `/?repo=${encodedRepo}&branch=${encodedBranch}`;
...
function parseURL() {
    ...
    if (repoFromUrl) {
        ...
        // Fetch only if repo/branch changed or data isn't cached
        if (currentRepo !== repoFromUrl || currentBranch !== decodedBranch || !cachedTreeData) {
            ...
            // If data is cached, just ensure metadata is correct for the current view
...
async function fetchRepoTree() {
    ...
    try {
        ...
        // Construct ENCODED canonical path for URL and meta tags using actual data
        const encodedRepo = encodeURIComponent(currentRepo); // Use currentRepo state
        const encodedBranch = encodeURIComponent(currentBranch); // Use currentBranch state
        const canonicalPath = `/?repo=${encodedRepo}&branch=${encodedBranch}`;
        ...
        updateMetaData(optimisticTitle, optimisticDescription, canonicalPathOnError);
...
function attachCopyButtonListeners() {
    ...
    // Use a flag to prevent attaching multiple listeners if rebuild happens
    if (contentArea.dataset.copyListenerAttached === 'true') return;
    contentArea.dataset.copyListenerAttached = 'true';

    // Delegated listener on the content area
    contentArea.addEventListener('click', (event) => {
        // Find the closest copy button ancestor of the clicked element
        const button = event.target.closest('.copy-button');
        if (button) {
            const path = button.dataset.path; // Get the path from data attribute
            const iconElement = button.querySelector('i'); // Find the icon within the button
            if (path && iconElement) {
                // Trigger feedback using the icon element
